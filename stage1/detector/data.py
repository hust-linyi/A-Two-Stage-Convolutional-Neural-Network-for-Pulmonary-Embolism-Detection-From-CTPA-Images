...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None):
        ...
        '''
        augment the number of big size data, sizelim(6)*1, sizelim(30)*3, sizelim(60)*7
        '''
        if self.phase != 'test':
            self.bboxes = []
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                if not sample.shape[1] == sample.shape[2] == sample.shape[3] == 96:
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        '''
        target is the one anno of the image, shape is [z,y,x,d]
        bboxes is the whole anno of the image, shape is [n,[z,y,x,d]]
        '''
